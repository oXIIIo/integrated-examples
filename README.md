...
8. 实现了正常应用与 CDN 流量中转（基于 WebSocket/HTTPUpgrade/SplitHTTP/gRPC over TLS） 应用可同时使用，且使用 CDN 进行流量中转也能获取到客户端真实 IP。
...
8. 若 CDN 流量中转不支持 WebSocket/HTTPUpgrade 通过，请使用 SplitHTTP 传输方式的应用替换来解决。
9. 若使用不可信的 CDN 进行流量中转，推荐使用自带加密的 VMess 或 Shadowsocks 协议配置；否则推荐使用轻量的 VLESS 或 Trojan 协议配置。
10. 流量伪装与防探测网站可由其它 WEB 应用软件实现，其支持反代应用（反代 WebSocket、HTTPUpgrade、SplitHTTP、gRPC 及 H2C）与支持回落应用（提供 H2C server 及 HTTP/1.1 server 的 WEB 服务）等取决于自身，其配置自行参考 Caddy 或 Nginx 对应示例。
